...
//    If both the "f" and "b" conditions are true, return "FizzBuzz". In all other cases, return the string unchanged
    public String fizzString(String str) {
        ...
        else if(str.substring(0,1).equals("f") && str.substring(str.length()-1).equals("b")) return "FizzBuzz";
...
//    but not 6 5 7 or 5 5 7. However, with the exception that if "equalOk" is true, equality is allowed, such
//    as 5 5 7 or 5 5 5.
    public boolean inOrderEqual(int a, int b, int c, boolean equalOk) {
        if(equalOk && c >=b && b >=a)   return true;
...
//    the right digit.)
    public boolean shareDigit(int a, int b) {
        ...
        int db = b/10;
        int mb = b%10;
        if(da == db || da == mb)    return true;
        else if(ma == db || ma == mb)   return true;
        return false;
    }
...
//    a non-negative int n is to convert it to a string with String.valueOf(n) and then check the length of the string.)
    public int sumLimit(int a, int b) {
        ...
        if(Ss.length() > Sa.length())  return a;
        return sum;
    }
